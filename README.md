...
## Environment preflight checks (Android Studio / CI)
Before opening in Android Studio (including Panda) or running CI builds, run:
...
This script checks:
- Java/Gradle environment visibility
- Merge conflict markers in core project files
- Firebase `google-services.json` flavor placement (`prod` / `dev` / `staging`)
- Package-name consistency for installed Firebase files
- Gradle task resolution smoke test (with JDK 21 fallback when available)

Use it as a quick “works in this environment” sanity check before full compile or device/emulator runs.
